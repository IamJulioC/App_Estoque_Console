...
def retirar_estoque(produtos)
  ...
  unless produto
    limpar_tela
    mensagem_vermelha("Produto do ID (#{id}) não encontrado na lista", false, false)
  ...
  limpar_tela
  mensagem_azul("Qual a quantidade deseja retirar do estoque do produto #{amarelo(produto[:nome])}?", false, false)
